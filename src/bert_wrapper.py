...
class BertForSentenceClassification(PreTrainedModel):
    ...
    def __init__(self, config, model_name, num_labels, class_weights=None, temperature=1.0):
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        return SequenceClassifierOutput(loss=loss, logits=scaled_logits)
...
class TrainerHandler:
    ...
    def compute_nll(self, batch_size=32):
        self.trainer.model.eval()
        device = self.trainer.model.device  # Ensure model and data are on the same device

        # Convert Hugging Face dataset to a PyTorch DataLoader
        dataset = self.tokenized_datasets["validation"].with_format("torch")
        ...
        with torch.no_grad():
            print("Computing NLL in batches")
            for batch in tqdm(dataloader):
                inputs = batch["input_ids"].to(device)
                attention_mask = batch["attention_mask"].to(device) if "attention_mask" in batch else None
                labels = batch["label"].to(device)

                outputs = self.trainer.model(input_ids=inputs, attention_mask=attention_mask)
                logits = outputs.logits if hasattr(outputs, "logits") else outputs

                log_probs = F.log_softmax(logits, dim=1)

                per_example_nll = F.nll_loss(log_probs, labels, reduction="none")

                total_nll += per_example_nll.sum().item()
                total_samples += labels.size(0)

        # Compute final NLL
        self.nll = total_nll / total_samples
        print(f"Negative Log-Likelihood: {self.nll}")
